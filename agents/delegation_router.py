...
CATEGORY_KEYWORDS: Dict[TaskCategory, List[str]] = {
    TaskCategory.RESEARCH: [
        "research", "search", "find information", "look up", "discover",
        "compare", "investigate", "study", "survey", "benchmark",
        "papers", "articles", "documentation", "read",
    ],
    TaskCategory.CODING: [
        "implement", "create", "write code", "build", "develop",
        "add feature", "implement", "script", "function", "class",
        "refactor", "generate code", "python", "javascript",
    ],
    TaskCategory.DEBUGGING: [
        "debug", "fix", "troubleshoot", "error", "broken", "not working",
        "exception", "traceback", "bug", "crash", "fails",
    ],
    TaskCategory.REVIEW: [
        "review", "audit", "check code", "code review", "quality",
        "security review", "best practices", "lint", "static analysis",
    ],
    TaskCategory.PLANNING: [
        "plan", "design", "architect", "spec", "roadmap",
        "break down", "decompose", "strategy", "approach",
    ],
    TaskCategory.DATA_ANALYSIS: [
        "analyze data", "data", "csv", "dataset", "statistics",
        "chart", "graph", "visualization", "plot", "report",
    ],
    TaskCategory.SECURITY: [
        "security", "vulnerability", "scan", "audit", "exploit",
        "penetration", "secret", "credential", "auth", "token",
    ],
    TaskCategory.WEB_BROWSING: [
        "browse", "website", "webpage", "click", "navigate",
        "scrape", "extract from page", "fill form", "screenshot",
    ],
    TaskCategory.VISUAL: [
        "image", "screenshot", "photo", "picture", "visual",
        "diagram", "chart", "draw", "design",
    ],
}
...
def _call_llm(prompt: str, system_prompt: str = "", timeout: int = 60) -> Optional[str]:
...
class DelegationRouter:
    ...
    def _save_task_log(self) -> None:
        ...
        with open(TASK_LOG_PATH, "w") as f:
            json.dump({"tasks": self._task_log[-500:], "total": len(self._task_log)}, f, indent=2)
    ...
    def route(
        ...
        task_text: str,
        dry_run: bool = False,
        on_assign: Optional[Callable[[SubTask, AgentEntry], None]] = None,
    ) -> Dict[str, Any]:
