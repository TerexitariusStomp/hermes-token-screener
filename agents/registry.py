...
class AgentRegistry:
    ...
    def register(
        ...
        agent = AgentEntry.create(name, role=role, model=model,
    ...
    def list_agents(
        ...
        if active_only:
            agents = [a for a in agents if a.status not in (
    ...
    def find_by_capability(self, capability: str) -> List[AgentEntry]:
        ...
        for agent in self._agents.values():
            ...
            if capability in caps.supported_toolsets:
                results.append(agent)
            elif capability == "browse" and caps.can_browse:
                results.append(agent)
            elif capability == "code" and caps.can_execute_code:
                results.append(agent)
    ...
    def bootstrap_default_agents(self) -> List[AgentEntry]:
        ...
        defaults = [
            ("researcher", AgentRole.RESEARCHER, "Research and web-based information gathering"),
            ...
            ("planner", AgentRole.PLANNER, "Task decomposition and long-horizon planning"),
            ("security", AgentRole.SECURITY_AGENT, "Security auditing and vulnerability scanning"),
        ]
        created = []
...
def main() -> None:
    ...
    p_select.add_argument("--strategy", default=RoutingStrategy.LEAST_LOADED,
    ...
    if args.command == "list":
        for entry in registry.list_all():
            print(f"  {entry['agent_id']:8s}  {entry['name']:12s}  {entry['role']:15s}"
