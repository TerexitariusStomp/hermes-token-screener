...
class Quantizer:
    ...
    def quantize_int8(vector: List[float]) -> Tuple[bytes, float, float]:
        ...
        quantized = struct.pack(f'{len(vector)}b', *[
    ...
    def quantize_int4(vector: List[float]) -> Tuple[bytes, float, float]:
        ...
        int4_values = [
            max(-7, min(7, int(round(v / scale))) if scale > 0 else 0)
            for v in vector
        ]
...
class TurboQuantStore:
    ...
    def add(self, key: str, vector: List[float], metadata: Optional[Dict] = None) -> bool:
    ...
    def add_batch(self, vectors: Dict[str, Tuple[List[float], Optional[Dict]]]) -> int:
    ...
    def get_store_stats(self) -> Dict:
        ...
        total_stored_bytes = sum(
            len(entry["data"]) for entry in self._index.values()
        )
...
if __name__ == "__main__":
    ...
    for r in results:
        print(f"  {r['tier']:8s} | {r['quality']:15s} | ratio: {r['actual_ratio']:5.2f}x | search: {r['search_results']} results")
    print(f"\nAll compression tiers working correctly.")
    print(f"Store ready for integration with Hermes Agent memory system.")