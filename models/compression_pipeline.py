...
class CompressionPipeline:
    ...
    def __init__(self, profile: str = "balanced", store_path: Optional[str] = None):
        ...
        import sys
        sys.path.insert(0, str(Path(__file__).parents[1]))  # Add ~/.hermes/ to path
        from memory.vector_store import TurboQuantStore, CompressionTier
    ...
    def compress_vectors(self, vectors: Dict[str, List[float]], 
                        metadata: Optional[Dict[str, Dict]] = None,
                        target_memory_mb: Optional[float] = None) -> Dict:
    ...
    def save_profile(self, name: str, config: Optional[Dict] = None):
...
def validate_pipeline():
    ...
    for tier, metrics in bench.items():
        print(f"    {tier:8s} | {metrics['compression_ratio']:5.2f}x | add: {metrics['add_time_ms']:7.2f}ms | search: {metrics['search_time_ms']:7.2f}ms")
    
    print(f"\nAll pipeline components working correctly.")
    print(f"Pipeline ready for Hermes Agent integration.")
    return result
