...
BEARISH_WORDS = {
    ...
    'ponzi', 'honeypot', 'honeypotted', 'stuck', 'locked', 'trapped',
    'fake', 'fraud', 'steal', 'stole', 'stolen', 'exploit', 'hack', 'hacked',
    'exit scam', 'dev sold', 'team dumped', 'abandoned', 'abandon',
    'overvalued', 'overhyped', 'hype', 'shitcoin', 'shit', 'trash', 'garbage',
...
NEUTRAL_WORDS = {
    ...
    'chart', 'price', 'market', 'trading', 'analysis', 'update',
    'contract', 'address', 'token', 'coin', 'crypto',
}
...
def compute_tweet_sentiment(tweets: list) -> dict:
    ...
    if not tweets:
        return {'sentiment_score': None, 'sentiment_label': '-', 'bullish': 0, 'bearish': 0, 'neutral': 0}
...
def main():
    ...
    for sym in sorted(tg.keys(), key=lambda s: tg[s]['rank']):
        ...
        search_score = max(ticker_search.get('search_quality_score', 0),
        ...
        print(f"#{t['rank']:>2} {sym:>10}  TG:{t['members']:>5}mbr {tg_s}({t['sentiment_label'][:4]:4})  "
    ...
    print(f"{'='*95}")
    print(f"{'Rank':>4} {'Sym':>10} {'TG_Mbr':>7} {'TG_Sent':>8} {'TW_Fll':>7} {'TW_Prof':>7} "
    ...
    for r in results:
        ...
        print(f"{r['rank']:>4} {r['symbol']:>10} {r['telegram_members']:>7,} {tg_s:>8} "
