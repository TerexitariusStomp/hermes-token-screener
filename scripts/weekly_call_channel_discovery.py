...
class CallChannelTracker:
            ...
            if not bot_response:
                ...
                for msg in messages_after:
                    if msg.message and len(msg.message) > 50:
                        if any(keyword in msg.message.lower() for keyword in ['leaderboard', 'best', 'trending', 'runners', 'hot', 'tokens']):
                            bot_response = msg.message
                            break
    ...
    def parse_ga_response(self, response: str) -> List[Dict]:
        ...
        for line in lines:
            ...
            if '➜' in line and 'Δ' in line:
                token_match = re.search(r'([A-Za-z0-9]+) @ ([0-9.]+[KMB]?) ➜ ([0-9.]+[KMB]?) Δ ([0-9.]+x?)', line)
                if token_match:
                    token_name = token_match.group(1)
                    first_cap = token_match.group(2)
                    ath_cap = token_match.group(3)
    ...
    def parse_burp_response(self, response: str) -> List[Dict]:
        ...
        for line in lines:
            ...
            if '➜' in line and 'Δ' in line:
                token_match = re.search(r'([A-Za-z0-9\u4e00-\u9fff]+) @ ([0-9.]+[KMB]?) ➜ ([0-9.]+[KMB]?) Δ ([0-9.]+x?)', line)
                if token_match:
                    token_name = token_match.group(1)
                    first_cap = token_match.group(2)
                    ath_cap = token_match.group(3)
        ...
        for command, description in BOT_COMMANDS:
            ...
            if response:
                ...
                cursor.execute('''
                    ...
                    VALUES (?, ?)
                ''', (command, json.dumps({'response': response, 'description': description})))
                self.db_conn.commit()
    ...
    async def enrich_tokens_with_details(self, tokens: List[Dict]) -> List[Dict]:
        ...
        for i, token in enumerate(tokens[:5]):  # Top 5 tokens
            if 'address' in token and token['address']:
                try:
                    response = await self.send_command(f"/soc {token['address']}", f"Find socials for {token.get('name', 'unknown')}")
                    if response:
                        social_links = self.extract_social_links(response)
                        if social_links['telegram']:
                            token['social_links'] = social_links
                            print(f"  Found {len(social_links['telegram'])} Telegram links for {token.get('name', 'unknown')}")
    ...
    def generate_weekly_report(self, tokens: List[Dict], channel_performance: Dict) -> str:
        ...
        # Sort tokens by gain
        sorted_tokens = sorted(tokens, key=lambda x: float(re.search(r'([0-9.]+)', x.get('gain', '0x')).group(1)) if re.search(r'([0-9.]+)', x.get('gain', '0x')) else 0, reverse=True)
        
        for i, token in enumerate(sorted_tokens[:10], 1):
            report_lines.append(f"{i:2d}. {token.get('name', 'Unknown'):15} | Gain: {token.get('gain', 'N/A'):10} | Source: {token.get('source', 'Unknown')}")
        ...
        if channel_performance:
            best_source = max(channel_performance.items(), key=lambda x: x[1]['total_gain'] / x[1]['count'] if x[1]['count'] > 0 else 0)
    ...
    async def run_weekly_collection(self):
        ...
        try:
            ...
            cursor.execute('''
                INSERT INTO weekly_reports (report_type, data_json)
                VALUES (?, ?)
            ''', ('weekly_report', json.dumps({'report': report, 'timestamp': datetime.now().isoformat()})))
            self.db_conn.commit()
