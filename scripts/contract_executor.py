...
_WP_PATH = _os.path.join(
    _os.path.dirname(_os.path.abspath(__file__)), "working_protocols.json"
)
try:
...
class ContractExecutor:
    ...
    def smart_swap(
        ...
        for proto in ["kyberswap", "velora", "odos", "1inch", "paraswap"]:
            ...
            if proto in api_routes:
                ...
                # Use value from route (transactionValue from API build),
                # NOT amount_in — the route was built with specific value expectations
                route_value = route.get("value", 0)
                if isinstance(route_value, str):
                    route_value = (
                        int(route_value, 16)
                        if route_value.startswith("0x")
                        else int(route_value)
                    )
