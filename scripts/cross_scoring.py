...
def _compute_token_composite_score(
    ...
    """
    Compute composite token score using ALL enrichment data + smart money signals.

    Weights (sum = 100):
      Smart Money Presence   30  (how many top wallets, their quality)
      Volume & Liquidity     20  (FDV, volume_h24, volume_h1)
      Price Momentum         15  (price_change_h1, price_change_h6)
      Social Signal          10  (channel_count, social_score)
      Token Fundamentals     10  (age_hours, gmgn_dev_hold, negatives)
      Security               10  (risk scores, honeypot flags)
      Freshness               5  (new tokens get a small bonus)
    """
    score = 0.0
    ...
    if max_smart_wallets > 0:
        ...
        if insider_count > 0:
            score += min(insider_count * 3, 5)
        if sniper_count > 0:
            score += min(sniper_count * 1.5, 3)
    ...
    if p1h is not None:
        if 5 <= p1h <= 50:
            score += 6  # strong but not parabolic
        elif 0 < p1h < 5:
            score += 4  # steady
        elif -10 < p1h <= 0:
            score += 2  # slight dip
        elif p1h > 50:
            score += 3  # parabolic (risky)
        else:
            score += 0  # dumping
    ...
    # ── Social Signal (0-10) ──
    channels = token.get("channel_count") or 0
    social = token.get("social_score") or 0
    ...
    penalty = len(negatives) * 1.5
    score -= min(penalty, 5)

    # ── Security (0-10) ──
    risk = token.get("cs_risk_score")
    if risk is not None and risk < 30:
        score += 5
    ...
    if rugcheck and rugcheck.get("lp_locked"):
        score += 2

    # ── Freshness (0-5) ──
    if 0 < age < 6:
        score += 5  # very new = high risk/reward
    elif 6 <= age < 24:
        score += 3
    elif 24 <= age < 48:
        score += 1
