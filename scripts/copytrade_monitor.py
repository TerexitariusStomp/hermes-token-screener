...
def init_discoveries_db() -> sqlite3.Connection:
    ...
    # Migration: add missing columns to discovered_tokens for existing DBs
    _migrate_discovered_tokens_schema(conn)

    return conn


def _migrate_discovered_tokens_schema(conn):
    """Add missing columns to discovered_tokens table for existing databases."""
    cursor = conn.execute("PRAGMA table_info(discovered_tokens)")
    ...
    for col, col_type in migrations:
        if col not in existing:
            try:
                conn.execute(f"ALTER TABLE discovered_tokens ADD COLUMN {col} {col_type}")
