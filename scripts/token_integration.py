...
class TokenIntegrationPipeline:
    ...
    def merge_and_deduplicate(self, rick_tokens: List[Dict], telegram_tokens: List[Dict]) -> List[Dict]:
        ...
        log.info(f"Merged {len(merged_list)} unique tokens (Rick: {len(rick_tokens)}, Telegram: {len(telegram_tokens)})")
    ...
    def run(self, max_enrich: int = 50):
        ...
        try:
            ...
            for i, token in enumerate(prioritized_tokens[:10]):
                print(f"{i+1:2d}. {token.get('name', 'Unknown'):15} | Score: {token.get('priority_score', 0):3d} | {token.get('priority_reason', '')}")
