...
def enrich_wallet(w: dict) -> dict:
    ...
    elif sol > 0.1: score += 10
    else: score += 5

    if bonded_count > 0: score += 15 * bonded_count
    if best_mcap > 1000000: score += 20
    elif best_mcap > 100000: score += 15
    elif best_mcap > 10000: score += 10
...
def main():
    ...
    for i, w in enumerate(batch):
        ...
        e = enriched[-1]
        print(f"  SOL={e['sol_balance']:.2f}  bonded={e.get('bonded_tokens',0)}  "
    ...
    for i, w in enumerate(enriched[:15], 1):
        print(f"{i:>4} {w['wallet'][:22]:>22} {w['token_count']:>4} {w['sol_balance']:>7.2f} "
