...
# ── Sentiment Keywords ──────────────────────────────────────────────────────
BULLISH_WORDS = {
    'bullish', 'moon', 'mooning', 'pump', 'pumping', 'gem', '100x', '10x', '1000x',
    'buy', 'buying', 'bought', 'long', 'hodl', 'hold', 'holding', 'accumulate',
    'rocket', 'lfg', 'wagmi', 'based', 'alpha', 'early', 'undervalued',
    'breakout', 'rally', 'surge', 'explodes', 'exploding', 'soaring', 'skyrocket',
    'massive', 'huge', 'incredible', 'amazing', 'love', 'great', 'strong',
    'diamond hands', 'to the moon', 'next big thing', 'free money',
    'partnership', 'listing', 'listed', 'announcement', 'launch', 'launched',
    'green', 'up', 'rising', 'gains', 'profit', 'winner', 'winning',
}
BEARISH_WORDS = {
    'bearish', 'dump', 'dumping', 'rug', 'rugpull', 'rug pull', 'scam', 'scammer',
    'sell', 'selling', 'sold', 'short', 'rekt', 'ngmi', 'dead', 'dying',
    'crash', 'crashing', 'tank', 'tanking', 'falling', 'collapse',
    'ponzi', 'honeypot', 'fake', 'fraud', 'steal', 'stolen', 'exploit', 'hack', 'hacked',
    'exit scam', 'dev sold', 'team dumped', 'abandoned', 'abandon',
    'overvalued', 'overhyped', 'shitcoin', 'shit', 'trash', 'garbage',
    'red', 'down', 'falling', 'loss', 'losses', 'bag', 'bagholder',
    'avoid', 'warning', 'careful', 'risky', 'danger', 'suspicious',
}
...
def compute_sentiment(tweets: list) -> dict:
    ...
    if not tweets:
        return {'sentiment_score': None, 'sentiment_label': '-', 'bullish': 0, 'bearish': 0, 'neutral': 0, 'analyzed': 0}
    ...
    if score > 60: label = 'very_positive'
    elif score > 20: label = 'positive'
    elif score > -20: label = 'neutral'
    elif score > -60: label = 'negative'
    else: label = 'very_negative'

    return {'sentiment_score': score, 'sentiment_label': label,
            'bullish': bull, 'bearish': bear, 'neutral': neutral, 'analyzed': n}
...
def analyze_profile(handle: str, page) -> ProfileAnalysis:
    ...
    for t in tweets:
        ...
        if d is not None:
            if d <= 7: a.tweets_last_7d += 1
            if d <= 30: a.tweets_last_30d += 1
            all_ages.append(d)
    ...
    if a.account_age_days > 365: score += 15
    elif a.account_age_days > 90: score += 10
    if a.posting_freq_per_day >= 1: score += 15
    elif a.posting_freq_per_day >= 0.3: score += 10
    if a.avg_likes > 100: score += 20
    elif a.avg_likes > 10: score += 10
    elif a.avg_likes > 0: score += 5
    if a.tweets_last_7d >= 3: score += 20
    elif a.tweets_last_7d >= 1: score += 10
    if a.total_tweets > 100: score += 5
...
def main():
    ...
    with sync_playwright() as p:
        ...
        for i, tok in enumerate(tokens):
            ...
            analysis = TokenTwitterAnalysis(
                symbol=sym, name=name, contract_address=contract, chain=chain,
            ...
            if handle:
                ...
                p = analysis.profile
                print(f"    {p.display_name}  followers={p.followers:,}  tweets={p.total_tweets}  "
            ...
            if goto_nitter(page, f'/search?f=tweets&q={re.escape(ticker_q)}'):
                ...
                print(f"    {ts.tweets_found} tweets  {ts.unique_authors} authors  "
            ...
            if name and name != sym and len(name) > 2:
                ...
                if goto_nitter(page, f'/search?f=tweets&q={name.replace(" ", "+")}'):
                    ...
                    print(f"    {ns.tweets_found} tweets  {ns.unique_authors} authors  "
            ...
            print(f"  Sentiment: {ss} ({sl})  [{sent['bullish']}B/{sent['bearish']}R/{sent['neutral']}N of {sent['analyzed']}]" if ss is not None else f"  Sentiment: no data")

            analysis.combined_score = round(
                0.40 * analysis.profile.profile_quality_score +
                0.35 * analysis.ticker_search.search_quality_score +
                0.25 * analysis.name_search.search_quality_score, 1
            )
    ...
    print(f"{'='*88}")
    print(f"{'Sym':>10} {'Profile':>10} {'Follow':>8} {'Twt':>5} {'7d':>4} {'AvgLike':>8} "
    ...
    for r in sorted(results, key=lambda x: -x['combined_score']):
        ...
        print(f"{r['symbol']:>10} {p['profile_quality_score']:>9.1f} {p['followers']:>8,} "
