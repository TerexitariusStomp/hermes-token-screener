...
class EnhancedTokenDiscovery:
            ...
            if not bot_response:
                ...
                for msg in messages_after:
                    if msg.message and len(msg.message) > 50:
                        if any(keyword in msg.message.lower() for keyword in ['trending', 'best', 'runners', 'popular', 'hot']):
                            bot_response = msg.message
                            break
    ...
    def extract_token_names(self, response: str) -> List[str]:
        ...
        common_words = {'the', 'and', 'for', 'with', 'this', 'that', 'from', 'have', 'been', 'will', 'your', 'more'}
        filtered_names = []
    ...
    def generate_report(self) -> str:
        ...
        sorted_tokens = sorted(tokens_with_liquidity, key=lambda x: float(x.get('liquidity', 0)), reverse=True)
