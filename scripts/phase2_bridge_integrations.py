...
try:
    ...
    paths = [x['path'] for x in tree.get('tree', []) if x.get('path','').endswith('/CBridge.json') and x.get('path','').startswith('deployments/')]
    for p in paths:
...
try:
    hop_ts = http_get('https://raw.githubusercontent.com/hop-protocol/hop/develop/packages/sdk/src/addresses/mainnet.ts', timeout=60)
...
ALIASES = {
    'bsc':'bnb smart chain','binance':'bnb smart chain','ethereum':'ethereum mainnet','mainnet':'ethereum mainnet',
    'arbitrum':'arbitrum one','optimism':'op mainnet','polygon':'polygon mainnet','avalanche':'avalanche c-chain',
    'xlayer':'x layer mainnet','worldchain':'world chain','seievm':'sei evm','klaytn':'kaia mainnet',
    'fantom':'fantom opera','metis':'metis andromeda mainnet','zksync':'zksync era','zksync era':'zksync era mainnet',
