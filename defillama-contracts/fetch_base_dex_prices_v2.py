#!/usr/bin/env python3
"""
Fetch prices from Base DEX contracts with rate limiting handling.
...
BASE_RPCS = [
    ...
    "https://mainnet.base.org",  # Last resort - has rate limits
]
...
def connect_with_fallback():
    """Connect to Base RPC with fallback."""
...
def main():
    ...
    print(f"Using {len(BASE_RPCS)} RPC endpoints with fallback")
    
    # Connect to Base RPC
    w3, rpc_url = connect_with_fallback()
    if not w3:
    ...
    for result in all_results:
        if "error" not in result:
            ...
            if successful_prices:
                successful += 1
                prices_by_protocol[result["protocol"]] = []
                
                print(f"\n{result['protocol']}:")
                for pair_name, price_info in prices.items():
                    if "price" in price_info:
                        price_str = f"1 {price_info['token_a']} = {price_info['price']} {price_info['token_b']}"
                        print(f"  {pair_name}: {price_str}")
                        prices_by_protocol[result["protocol"]].append({
                            "pair": pair_name,
                            "price": price_info["price"],
                            "token_a": price_info["token_a"],
                            "token_b": price_info["token_b"]
                        })
                
                # Show top pool info
                if result.get("top_pool") and "reserves" in result["top_pool"]:
                    reserves = result["top_pool"]["reserves"]
                    print(f"  Top pool reserves: {reserves['reserve0']} {reserves['token0']} / {reserves['reserve1']} {reserves['token1']}")
            else:
    ...
    print("=== Final Summary ===")
    print(f"{'='*60}")
    print(f"Total routers tested: {len(all_results)}")
    print(f"Successful: {successful}")
    print(f"Failed: {len(all_results) - successful}")
    
    if prices_by_protocol:
        print(f"\nPrices fetched:")
        for protocol, prices in prices_by_protocol.items():
            print(f"  {protocol}:")
            for p in prices:
                print(f"    {p['pair']}: {p['price']} {p['token_b']}")
