...
# === END SINGLE INSTANCE LOCK (functions only, no side effects at import time) ===
...
class DexAggregatorTrader:
    ...
    def sell_token_for_eth(
        self, token_name: str, token_address: str, sell_pct: float = 0.3
    ) -> bool:
        ...
        try:
            ...
            quote_resp = requests.post(
                ...
                json={
                    "chainId": 8453,
                    "inputTokens": [
                        {"tokenAddress": token_address, "amount": amount_wei}
                    ],
                    "outputTokens": [
                        {
        ...
        while True:
            try:
                ...
                if base_bal < Decimal("0.0003") and holdings:
                    ...
                    for tok_name, tok_info in holdings.items():
                        ...
                        sold = self.sell_token_for_eth(
                            tok_name, tok_info["address"], sell_pct=0.3
                        )
                        if sold:
                ...
                if base_bal < Decimal("0.0001") and sol_bal < Decimal("0.05"):
                    ...
                    logger.info(
                        f"Base balance low ({base_bal:.6f} ETH) and no token holdings to sell"
                    )
